#!/usr/bin/env python3
"""Одноразовый скрипт: аватарка для David Key (@david_key_bot)."""
...
BOT = {
    "username": "@david_key_bot",
    "prefix": "david_key",
    "prompt": (
        "Professional portrait of David Key, a confident and charismatic NLP coach and hypnotherapist "
        "in his mid-40s, warm approachable smile with knowing eyes, well-groomed salt-and-pepper stubble, "
        "smart casual blazer over open collar shirt, clean modern background with soft neutral tones, "
        "trustworthy and transformative energy, high quality photography, realistic, 8k"
    ),
}
...
async def main():
    ...
    async with TelegramClient(str(CHATS_DIR / "session.session"), int(creds["TELEGRAM_API_ID"]), creds["TELEGRAM_API_HASH"]) as client:
