...
### Changed
- `chatcore/llm.py`: `_resolve_backend()` при `LLM_BACKEND=auto`, когда ни
  один upstream (cliproxy/grok/claude-cli/claude) не сконфигурирован, теперь
  пишет `WARNING` с перечислением причины отказа каждого из четырёх путей
  перед фолбэком в `ollama` — раньше это событие ничем не отличалось от
  штатной работы (`grep -i warning` его не находил), из-за чего дефект на
  проде (VPS Contabo, шесть ботов, один жил в этом состоянии с 25 июля)
  обнаружился только по прямой жалобе пользователя, а не по логам (#30).

  **Для операторов:** если хост осознанно работает на Ollama (например, нет
  cliproxy/grok/claude-cli credentials и это ожидаемо), задайте
  `LLM_BACKEND=ollama` явно в `.env` — WARNING в этом случае не пишется.
  Без явного значения (`auto`/не задано) при decorative-cascade состоянии
  WARNING будет появляться на каждое сообщение — это осознанно (см. issue),
  не троттлится.

  **Важно:** правка живёт в `chatcore`, боты-потребители получат её только
  после обновления зависимости и передеплоя/рестарта — сама по себе она не
  долетает до уже работающих процессов.
