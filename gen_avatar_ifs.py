#!/usr/bin/env python3
"""Одноразовый скрипт: аватарка для IFS Guide (@ifs_course_bot)."""
...
BOT = {
    "username": "@ifs_course_bot",
    "prefix": "ifs",
    "prompt": (
        "Serene spiritual guide portrait, soft violet and deep purple tones, "
        "abstract human silhouette in meditative pose, gentle inner light glowing from center of chest, "
        "surrounding fractal mandala-like inner world with small luminous figures representing parts, "
        "calm sacred energy, IFS Internal Family Systems concept art, "
        "smooth gradient background deep purple #5b3fa8 to soft lavender, "
        "no text, no words, high quality digital art, 8k"
    ),
}
...
async def set_avatar(client, username, image_path):
    ...
    return "changed" in r3.lower() or "photo" in r3.lower()

async def main():
    creds = dotenv_values(CHATS_DIR / ".env")
    client = TelegramClient(
        str(CHATS_DIR / "session"),
    ...
    img = generate_image(BOT["prompt"], BOT["prefix"])
    if not img:
        print("  ✗ Не удалось сгенерировать аватарку")
    else:
        print(f"  ✓ Сохранена: {img}")
        ok = await set_avatar(client, BOT["username"], img)
        print("  ✓ Аватарка установлена" if ok else "  ✗ Не удалось установить аватарку")
