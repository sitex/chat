...
def build_persona_prompt(persona: dict, personas: list[dict], topic: str) -> str:
    ...
    return "\n".join([
        ...
        "- Коротко: 2–4 предложения. Не пересказывай чужие слова — реагируй: "
...
def build_turn_message(topic: str, transcript: list[tuple[str, str]], me: str) -> str:
    ...
    if transcript:
        ...
        lines.append("")
        lines.append(f"Сейчас твоя очередь, {me}. Дай свою реплику.")
    else:
        lines.append(f"Ты открываешь обсуждение, {me}. Дай свою первую реплику по теме.")
    return "\n".join(lines)
