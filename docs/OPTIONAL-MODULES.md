...
## Встроенные модули (уже в chatcore)

| Модуль | Что делает |
|--------|-----------|
| `chatcore/ratelimit.py` | Sliding-window rate-limiter для `on_message` (10 сообщ./60 с на пользователя; `rate_limit=0` — отключить; in-memory, перезапуск сбрасывает окна) |
| `chatcore/admin.py` | `/admin_status`, `/admin_stats`, `/admin_reset <chat_id>` — по `ADMIN_IDS` из env; молчаливый игнор не-админов; в меню команд не показывается |

---

# Планируемые модули (план реализации)
