...
## 7. Rate-limiting и admin-команды

### Rate-limiting

`scaffold.run()` принимает два необязательных параметра:

```python
scaffold.run(
    ...,
    rate_limit=10,   # макс. сообщений от одного пользователя за окно (0 — отключить)
    rate_period=60.0,  # длина скользящего окна в секундах
)
```

- Лимитируется только диалог (`on_message`); команды (`/start`, `/help`, контент-команды) не лимитируются.
- При превышении пользователь получает **одно** вежливое предупреждение; следующие сообщения молча игнорируются до конца окна.
- После сдвига окна диалог возобновляется автоматически.
- `rate_limit=0` полностью отключает лимитирование.
- ⚠️ Счётчики хранятся в памяти процесса — перезапуск бота сбрасывает окна.

### Admin-команды

Добавьте `ADMIN_IDS` в `.env`:

```
ADMIN_IDS=123456789,987654321
```

Владельцы бота (Telegram `user_id`) получают три команды:

| Команда | Что делает |
|---------|-----------|
| `/admin_status` | Имя бота, версия chatcore, LLM-бэкенд, uptime, число чатов в БД |
| `/admin_stats` | Топ чатов по user-сообщениям за 24ч и 7д |
| `/admin_reset <chat_id>` | Сбросить историю и seen-state указанного чата |

**Важно:**
- Не-админам команды **не отвечают** (молчаливый игнор — не раскрываем существование).
- Admin-команды **не отображаются** в меню команд Telegram.
- `ADMIN_IDS` опционален: без него команды молчат для всех.

## 8. Тестирование и инструменты разработки
